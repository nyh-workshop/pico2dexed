...
private:
    static const int8_t MAX_CHANNELS = 32;
    int8_t noteAdjust = 0;
...
void PlayTune<T>::stepScore()
{
  ...
  */
    ...
    while (tune_playing)
    {
        ...
        { /* stop note */
            //mutexTryEnter();
            if(ns[chan].isPlaying)
            {
                noteOff(ns[chan].noteNum);
                ns[chan].isPlaying = false;
            }
        ...
        {                                 /* play note */
            ...
            ns[chan].noteNum = note;
            ns[chan].isPlaying = true;
            if (volume_present)
